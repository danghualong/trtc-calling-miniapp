...
Page({
  ...
  onLoad: async function() {
    try{
      const res=await post("/callcenter/trtc/user_sig",{
      ...
      if(res.status){
        ...
        this.setData({
          ...
          this.TRTCCalling.login().then(()=>{
            this.call();
          });
        })
      }else{
        console.log(TAG_NAME,"无法获取用户签名");
      }
    }catch(err){
